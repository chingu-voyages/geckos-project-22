...
const ContactsListItem = ({
  contact: { contact_id, name, generalNote, pastMeetings },
  queue,
  editContact,
  deleteContact,
  addToQueue,
  ...
  const createLastContactInformation = () =>
    noOfMeetings
      ? `${
          pastMeetings[noOfMeetings - 1].note
        } - ${formatDate(
          pastMeetings[noOfMeetings - 1].timestamp,
        )}`
      : 'never';
  const findContactInQueue = () =>
    queue.find(
      (queueItem) => queueItem.contactId === contact_id,
    )
      ? true
      : false;
  ...
  return (
    <li className="contacts-list__item border-box">
      ...
      <button
        ...
        onClick={() => editContact(contact_id)}
      >
        Edit
      </button>
      <p>General Note: {generalNote || 'No Note'}</p>
      <p>Number of Past Meetings: {noOfMeetings}</p>
